        ...
        <header>
            <div className="flex flex-row p-5 items-center justify-between z-20 shadow-md bg-black/40 backdrop-blur-sm">
                <Link href={"/"} prefetch={false}>
                    <h1 className="font-bold text-lg ">NextFire</h1>
                </Link>
                    ...
                    <div className="hidden md:flex gap-2">
                        {route.map((item) => (
                            <Link key={item.name} href={item.link}>
                                <span className={pathname == item.link ? "my-activebtn" : "my-btn"}>{item.name}</span>
                            </Link>
                        ))}
                    </div>
                    <div className="ml-5 relative cursor-pointer">
                        {toggleMenu ? (
                            <XMarkIcon id="toogle-click" className="md:hidden h-8 w-8 cursor-pointer" />
                        ) : (
                            <Bars3Icon
                                id="toogle-click"
                        ...
                        {toggleMenu && (
                            <ul className="absolute flex justify-center align-middle p-4 top-10 right-0 bg-gray-900 rounded-xl shadow-xl flex-col gap-2 ">
                                {route.map((item) => (
                                    <Link key={item.name} href={item.link}>
                                        <li className={pathname == item.link ? "my-btn" : "my-activebtn"}>
                                            {item.name}
                                        </li>
                                    </Link>
                                ))}
